...
export default function AgregarKnowledge() {
  ...
  const handleSubmit = async e => {
    ...
    setMensaje('');
    try {
      const response = await fetch('http://localhost:5000/agregarKnowledge', {
        method: 'POST',
      ...
      if (response.ok) {
        setMensaje(data.message || 'Knowledge agregado exitosamente.');
        setForm({ cliente: '', servicio: '', fecha_salida: '', solucion: '' });
      } else {
        setMensaje(data.error || 'Error al agregar el knowledge.');
      }
    } catch (error) {
